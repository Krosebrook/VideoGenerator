...
export const generateTextImage = async ({ text, style, typographyPrompt, referenceImage, imageSize = '1K' }: TextImageOptions): Promise<{ data: string, mimeType: string }> => {
  const ai = getAI();
  const parts: any[] = [];
  ...
  if (referenceImage) {
    ...
    parts.push({ 
      text: `Analyze the environment, background, color palette, lighting, and textures of this reference image. 
      Create a NEW high-resolution cinematic keyframe featuring the text "${text}" written in the center. 
      The text should be constructed from materials that complement the environment in the reference image.
      Typography Instruction: ${typoInstruction}.
      Additional Style Direction: ${style}.
      Ensure the text is the focal point and the background captures the essence of the reference image provided.` 
    });
  } else {
    parts.push({ 
...
export const generateTextVideo = async (
  ...
  // Use 1920x1080 for 4k request as well, since it's the max API supported resolution, 
  // but we will use the higher quality model.
  const width = resolution === '720p' ? 1280 : 1920;
  const height = resolution === '720p' ? 720 : 1080;

  // Use the high-quality generation model for 4k requests to ensure premium output
  const model = resolution === '4k' ? 'veo-3.1-generate-preview' : 'veo-3.1-fast-generate-preview';
  const apiResolution = resolution === '720p' ? '720p' : '1080p';
